...
func ShortenURL(c *fiber.Ctx) error {
	...
	if !helpers.RemoveDomainError(body.URL) {
		return c.Status(fiber.StatusServiceUnavailable).JSON(fiber.Map{"error":"Infinite loop prevented"})
	}
	...
	if value != "" {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"error":"Custom URL short is already in use"})
	}
